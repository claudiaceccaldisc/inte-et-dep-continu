...
describe("RegisterForm", () => {
  ...
  test("affiche erreur si prénom vide", async () => {
    render(<RegisterForm addUser={jest.fn()} />);
    fireEvent.click(screen.getByRole("button"));

    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Le prénom est obligatoire");
  });
  ...
  test("affiche erreur si nom vide", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Le nom est obligatoire");
  });
  ...
  test("affiche erreur email invalide", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Email invalide");
  });
  ...
  test("affiche erreur si mineur", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Vous devez être majeur");
  });
  ...
  test("affiche erreur si ville vide", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("La ville est obligatoire");
  });
  ...
  test("affiche erreur si code postal invalide", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Code postal invalide");
  });
  ...
  test("inscription réussie", async () => {
    ...
    render(<RegisterForm addUser={addUserMock} />);
    ...
    await waitFor(() =>
      expect(screen.getByTestId("success"))
        .toHaveTextContent("Inscription réussie")
    );
  ...
  test("affiche erreur si API échoue", async () => {
    ...
    expect(await screen.findByTestId("error"))
      .toHaveTextContent("Erreur serveur");
  });
